...
export function makeVideoElementDisplayInterface(): DisplayInterface {
    ...
    function reportAudioTracks() {
        ...
        const languageTrack = audioTracks.find(
            (v) => v.lang === currentLanguage,
        );
        if (languageTrack) {
        ...
        if (src.type === "hls") {
            ...
            if (!hls) {
                ...
                hls.on(Hls.Events.MANIFEST_LOADED, () => {
                    ...
                    if (isExtensionActiveCached()) {
                        ...
                        hls.on(
                            ...
                            async (_, data) => {
                                const chunkUrlsDomains =
                                    data.details.fragments.map(
                                        (v) => new URL(v.url).hostname,
                                    );
                                const chunkUrls = [
                                    ...new Set(chunkUrlsDomains),
                                ];
                ...
                hls.on(Hls.Events.LEVEL_SWITCHED, () => {
                    if (!hls) return;
                    const quality = hlsLevelToQuality(
                ...
                hls.on(Hls.Events.SUBTITLE_TRACK_LOADED, () => {
                    for (const [lang, resolve] of languagePromises) {
                        const track = hls?.subtitleTracks.find(
                            (t) => t.lang === lang,
                        );
                        if (track) {
                            resolve();
