...
async function getMetas(
    ...
    for (const item of relevantItems.filter(Boolean)) {
        ...
        if (item.data.type === "show") {
            ...
            keys = seasons
                .map((season) =>
                    season ? [season.num, season?.season?.id] : [],
                )
    ...
    for (const oldWatched of oldData.items) {
        if (oldWatched.mediaType === "movie") {
            ...
            const newItem: WatchedStoreItem = {
                item: {
                    meta: mediaMetas[oldWatched.mediaId]["0"]
                        ?.meta as MWMediaMeta,
                },
            ...
            const newItem: WatchedStoreItem = {
                item: {
                    ...
                    series: {
                        ...
                        episodeId:
                            meta.seasonData.episodes[
                                Number(oldWatched.episodeId) - 1
                            ].id,
                    },
                },
                progress: oldWatched.progress,
            ...
            if (
                newData.items.find(
                    (item) =>
                        ...
                        item.item.series?.episodeId ===
                            newItem.item.series?.episodeId,
                )
            )
                continue;
